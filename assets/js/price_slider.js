...
class PriceSlider {
     ...
     * @param {String} prefix
     */
    constructor(el, config = {}, prefix = 'price_slider__') {
  
      this.#state = {
        prefix: prefix, // префикс для классов
        el: el, // элемент который нужно активировать как PriceSlider
    ...
    /**
     * Статический метод, который возвращает экземпляр PriceSlider, связанный с DOM-элементом
     ...
     * @param {String} prefix
     */
    static getOrCreateInstance(target, config = {}, prefix = 'price_slider__') {
      try {
    ...
    // статический метод для активирования элементов как PriceSlider на основе data-атрибутов
    static createInstances() {
      document.querySelectorAll('[data-slider="price_slider"]').forEach((el) => {
    ...
    #attachEvents() {
      this.#state.events = {
        ...
        'price_slider__transition-start': [this.#state.elItems, this.#onTransitionStart.bind(this), this.#config.loop],
    ...
    #move() {
      ...
      this.#state.elItems.dispatchEvent(new CustomEvent('price_slider__transition-start', {
        bubbles: true
      }));
    }
